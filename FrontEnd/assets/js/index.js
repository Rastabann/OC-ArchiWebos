    ...
    .then(function (response) {
      if (response.ok) {
        response.json().then(function (myJson) {
          ...
          photoGalleryTitle.style.fontSize = "24px";
          photoGalleryTitle.style.marginTop = "30px";
          photoGalleryTitle.style.marginBottom = "30px";
          photoGalleryWrapper.setAttribute("class", "wrapper");
          photoGalleryWrapper.setAttribute("id", "images");
          photoGalleryWrapper.setAttribute("style", "width: 70%; margin:auto;");
          ...
          addButton.setAttribute(
            "style",
            "padding:15px; border-style:none; cursor:pointer;"
          );
...
function deleteWork(id) {
  ...
  fetch(myRequest, myInit)
    ...
    .catch(function (error) {
      console.log(
        "Il y a eu un problème avec l'opération delete : " + error.message
      );
    });
}
...
function login() {
  ...
  if (localStorage["token"] != "null") {
    Array.from(document.getElementsByClassName("buttonEdition")).forEach(
      (element) => {
        element.style.display = "block";
      }
    );
    Array.from(document.getElementsByClassName("wrap-categories")).forEach(
      (element) => {
...
function lol() {
  ...
  fetch(myRequest, myInit)
    ...
    .catch(function (error) {
      console.log(
        "Il y a eu un problème avec l'opération ajout : " + error.message
      );
    });
}
